...
- `ScenePreviewRenderer.renderPreviewScenePass(...)`、`ScenePreviewStateScope`、`BlockPreviewScope`、`TileEntityPreviewScope`、`ActorPreviewStateScope`、`ActorBodyRenderer`、`ActorPreviewRenderData` 与 actor pose helper 已抽出，actor body 绘制 helper 已收口，preview 帧生命周期、主场景 pass、块模型预览 scope、tile entity preview scope、actor-pass scope、actor body dispatch、actor render data 与 `poseName` 分类已分层，`renderActorPreviews()` 的手写 blend 恢复已收口
...
- `ScenePreviewRenderer`、`ShowcaseRenderer`、`ShowcaseHudRenderer`、`DebugPanelRenderer`、`SceneOverlayRenderer`、`GuiOverlayRenderer` 已形成 renderer 分层，`BlockPreviewScope`、`TileEntityPreviewScope`、`ActorPreviewStateScope`、`ActorBodyRenderer`、`ActorPreviewRenderData` 与 actor pose helper 已落位，actor body 绘制 helper 已收口
...
- `ScenePreviewRenderer` 已把 preview frame lifecycle、scene pass、块模型预览 scope、tile entity preview scope、actor-pass scope、actor pose helper、actor body 绘制 helper、actor body dispatch 与 actor render data 分开，`BlockPreviewScope` 已把块模型预览状态收进局部 scope，`TileEntityPreviewScope` 已把 TileEntity 渲染状态收进局部 scope，`ActorPreviewStateScope` 已把 actor blend 状态恢复收进局部 scope，`BirbPoseKind` 已收口 `poseName` 判定
...
- `ScenePreviewRenderer` 的 preview scene pass、preview state scope、`BlockPreviewScope`、`TileEntityPreviewScope`、actor-pass scope、actor pose helper、actor body 绘制 helper、actor body dispatch 与 actor render data 已抽出，`renderActorPreviews()` 手写 blend 恢复已收口
...
5. `ScenePreviewRenderer` 和 snapshot renderer 接入 scoped scissor / matrix / depth guard，继续收口 preview GL / shadow、actor render dispatch 汇总层与其他自然切片
...
- `ScenePreviewRenderer` 已抽出 `renderPreviewScenePass(...)`、`ScenePreviewStateScope`、`BlockPreviewScope`、`TileEntityPreviewScope`、`ActorPreviewStateScope`、`ActorBodyRenderer`、`ActorPreviewRenderData` 与 actor pose helper，actor body 绘制 helper 已收口，`renderActorPreviews()` 手写 blend 恢复已收口
...
- 文档与 live 结构同步，包含 `BirbPoseKind` / `poseName` 分类、`ActorBodyDrawContext` 绘制上下文、`ActorBodyRenderer` 分派、`drawBirbBody`、`drawItemBody`、`drawCartWheels`、`ActorPreviewRenderData` 的归位
...
5. 继续推进 `ScenePreviewRenderer` 的 actor render dispatch 汇总层
...
5. ScenePreviewRenderer：推进 actor render dispatch 汇总层，继续收口 `renderActorPreviews()`、`renderActorPreview()`、`ActorPreviewRenderData` 与 `ActorBodyRenderer` 的编排边界
