...
- `ScenePreviewRenderer.renderPreviewScenePass(...)`、`ScenePreviewStateScope`、`BlockPreviewScope`、`TileEntityPreviewScope` 与 `ActorPreviewStateScope` 已抽出，preview 帧生命周期、主场景 pass、块模型预览 scope、tile entity preview scope、actor-pass scope 已分层，`renderActorPreviews()` 的手写 blend 恢复已收口
...
- `ScenePreviewRenderer`、`ShowcaseRenderer`、`ShowcaseHudRenderer`、`DebugPanelRenderer`、`SceneOverlayRenderer`、`GuiOverlayRenderer` 已形成 renderer 分层，`BlockPreviewScope`、`TileEntityPreviewScope` 与 `ActorPreviewStateScope` 已落位
...
- `ScenePreviewRenderer` 已把 preview frame lifecycle、scene pass、块模型预览 scope、tile entity preview scope 与 actor-pass scope 分开，`BlockPreviewScope` 已把块模型预览状态收进局部 scope，`TileEntityPreviewScope` 已把 TileEntity 渲染状态收进局部 scope，`ActorPreviewStateScope` 已把 actor blend 状态恢复收进局部 scope
...
- `ScenePreviewRenderer` 的 preview scene pass、preview state scope、`BlockPreviewScope`、`TileEntityPreviewScope` 与 actor-pass scope 已抽出，`renderActorPreviews()` 手写 blend 恢复已收口
...
- `ScenePreviewRenderer` 已抽出 `renderPreviewScenePass(...)`、`ScenePreviewStateScope`、`BlockPreviewScope`、`TileEntityPreviewScope` 与 `ActorPreviewStateScope`，`renderActorPreviews()` 手写 blend 恢复已收口
...
5. 继续拆分 `ScenePreviewRenderer` 的 actor body dispatch
...
5. ScenePreviewRenderer：拆分 actor body dispatch，把 birb / item / cart 绘制继续局部化
