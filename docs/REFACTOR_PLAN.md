...
- `ScenePreviewRenderer.renderPreviewScenePass(...)`、`ScenePreviewStateScope`、`BlockPreviewScope`、`TileEntityPreviewScope`、`ActorPreviewStateScope` 与 `ActorBodyRenderer` 已抽出，preview 帧生命周期、主场景 pass、块模型预览 scope、tile entity preview scope、actor-pass scope 与 actor body dispatch 已分层，`renderActorPreviews()` 的手写 blend 恢复已收口
...
- `ScenePreviewRenderer`、`ShowcaseRenderer`、`ShowcaseHudRenderer`、`DebugPanelRenderer`、`SceneOverlayRenderer`、`GuiOverlayRenderer` 已形成 renderer 分层，`BlockPreviewScope`、`TileEntityPreviewScope`、`ActorPreviewStateScope` 与 `ActorBodyRenderer` 已落位
...
- `ScenePreviewRenderer` 已把 preview frame lifecycle、scene pass、块模型预览 scope、tile entity preview scope、actor-pass scope 与 actor body dispatch 分开，`BlockPreviewScope` 已把块模型预览状态收进局部 scope，`TileEntityPreviewScope` 已把 TileEntity 渲染状态收进局部 scope，`ActorPreviewStateScope` 已把 actor blend 状态恢复收进局部 scope
...
- `ScenePreviewRenderer` 的 preview scene pass、preview state scope、`BlockPreviewScope`、`TileEntityPreviewScope`、actor-pass scope 与 actor body dispatch 已抽出，`renderActorPreviews()` 手写 blend 恢复已收口
...
- `ScenePreviewRenderer` 已抽出 `renderPreviewScenePass(...)`、`ScenePreviewStateScope`、`BlockPreviewScope`、`TileEntityPreviewScope`、`ActorPreviewStateScope` 与 `ActorBodyRenderer`，`renderActorPreviews()` 手写 blend 恢复已收口
...
- 文档与 live 结构同步，包含 `ActorBodyRenderer` 分派、`drawBirbBody`、`drawItemBody`、`drawCartWheels` 的归位
...
5. 继续提取 `ScenePreviewRenderer` 的 actor pose / render data
...
5. ScenePreviewRenderer：提取 actor pose / render data，把 actor 状态、预计算颜色、当前 tick 与姿态判断继续局部化
