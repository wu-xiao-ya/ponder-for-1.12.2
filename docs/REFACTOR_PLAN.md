...
- `ScenePreviewRenderer.renderPreviewScenePass(...)`、`ScenePreviewStateScope`、`BlockPreviewScope`、`TileEntityPreviewScope`、`ActorPreviewStateScope`、`ActorPreviewRenderPass`、`ActorBodyRenderer`、`ActorPreviewRenderData` 与 actor pose helper 已抽出，actor body 绘制 helper 已收口，preview 帧生命周期、主场景 pass、块模型预览 scope、tile entity preview scope、actor-pass scope、actor render dispatch、actor body dispatch、actor render data 与 `poseName` 分类已分层，`renderActorPreviews()` 已收成 actor pass 入口
...
- `ScenePreviewRenderer` 已把 preview frame lifecycle、scene pass、块模型预览 scope、tile entity preview scope、actor-pass scope、actor render dispatch、actor pose helper、actor body 绘制 helper、actor body dispatch 与 actor render data 分开，`BlockPreviewScope` 已把块模型预览状态收进局部 scope，`TileEntityPreviewScope` 已把 TileEntity 渲染状态收进局部 scope，`ActorPreviewStateScope` 已把 actor blend 状态恢复收进局部 scope，`BirbPoseKind` 已收口 `poseName` 判定
...
- `ScenePreviewRenderer` 的 preview scene pass、preview state scope、`BlockPreviewScope`、`TileEntityPreviewScope`、actor-pass scope、`ActorPreviewRenderPass`、actor pose helper、actor body 绘制 helper、actor body dispatch 与 actor render data 已抽出，`renderActorPreviews()` 已收成 actor pass 入口
...
- line count 当前反映 adapter 与 renderer 过渡期成本，后续自然切片转向 actor 绘制原语下沉、appearance 策略分层与剩余 preview 桥接收口
...
5. `ScenePreviewRenderer` 和 snapshot renderer 接入 scoped scissor / matrix / depth guard，继续收口 preview GL / shadow、actor 绘制原语与 appearance 策略层
...
- `ScenePreviewRenderer` 已抽出 `renderPreviewScenePass(...)`、`ScenePreviewStateScope`、`BlockPreviewScope`、`TileEntityPreviewScope`、`ActorPreviewStateScope`、`ActorPreviewRenderPass`、`ActorBodyRenderer`、`ActorPreviewRenderData` 与 actor pose helper，actor body 绘制 helper 已收口，`renderActorPreviews()` 已收成 actor pass 入口
...
- 文档与 live 结构同步，包含 `ActorPreviewRenderPass`、`BirbPoseKind` / `poseName` 分类、`ActorBodyDrawContext` 绘制上下文、`ActorBodyRenderer` 分派、`drawBirbBody`、`drawItemBody`、`drawCartWheels`、`ActorPreviewRenderData` 的归位
...
5. 完成 `ScenePreviewRenderer` 的 actor render dispatch 汇总层，后续推进 actor 绘制原语下沉与 appearance 策略分层
...
5. ScenePreviewRenderer：推进 actor 绘制原语下沉，继续收口 `ActorBodyDrawContext`、`ActorPreviewRenderData`、`BirbPoseKind` 与 `ActorBodyRenderer` 的归属边界
