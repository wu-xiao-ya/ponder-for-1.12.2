...
final class EmbeddedReflectiveTabGuiSnapshot implements PonderGuiSnapshotRegistry.SnapshotRenderer {
    ...
    private void renderGui(Minecraft mc, SnapshotContext context, int x, int y, int width, int height, float currentTick)
        ...
        Object tab = getFieldValue(gui, tabFieldName);
        if (tab == null) {
            disabled = true;
            return;
        }
        ...
        invokeNoArg(tab, "update");

        int guiLeft = getIntField(gui, "guiLeft", "field_147003_i");
        int guiTop = getIntField(gui, "guiTop", "field_147009_r");
    ...
    private Rect getTabBounds(Object tab) {
        try {
            ...
            Object x = getFieldValue(bounds, "x");
