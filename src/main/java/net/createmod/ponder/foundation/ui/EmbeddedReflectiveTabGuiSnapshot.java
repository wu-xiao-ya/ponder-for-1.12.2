...
final class EmbeddedReflectiveTabGuiSnapshot implements SnapshotRenderer.GuiSnapshotRenderer {
    ...
    private final String tabFieldName;
    private final int panelWidth;
    private final int panelHeight;
    private SnapshotContext cachedContext;
    private boolean disabled;

    EmbeddedReflectiveTabGuiSnapshot(String guiClassName, String tileClassName, String tabFieldName, int panelWidth,
        int panelHeight) {
    ...
    public void render(int x, int y, int width, int height, float currentTick, float fade) {
        ...
        try {
            ...
            SnapshotTileSeedHelper.clearInventorySlots(context.tile, 16);
            ...
            cachedContext = null;
            disabled = true;
            Ponder.LOGGER.error("Failed to render embedded gui tab snapshot {} / {} / {}", guiClassName, tileClassName,
                tabFieldName, throwable);
        }
    }
    ...
    private void renderGui(Minecraft mc, SnapshotContext context, int x, int y, int width, int height, float currentTick)
        ...
        int guiWidth = Math.max(x * 2 + panelWidth, x + 240);
        int guiHeight = Math.max(y * 2 + panelHeight, y + 180);
        ...
        Object tab = SnapshotGuiReflectionHelper.getFieldValue(gui, tabFieldName);
        if (tab == null) {
            disabled = true;
            return;
        }
        SnapshotGuiReflectionHelper.invokeBoolean(tab, "setVisible", true);
        SnapshotGuiReflectionHelper.invokeNoArg(tab, "setFullyOpen");
        SnapshotGuiReflectionHelper.invokeNoArg(tab, "update");

        int guiLeft = SnapshotGuiReflectionHelper.getIntField(gui, "guiLeft", "field_147003_i");
        int guiTop = SnapshotGuiReflectionHelper.getIntField(gui, "guiTop", "field_147009_r");
        Rect tabBounds = getTabBounds(tab);
        if (tabBounds == null) {
            disabled = true;
            return;
        }
        ...
        SnapshotGuiLayoutHelper.setGuiContainerPosition(gui, x - tabOffsetX, y - tabOffsetY);
        SnapshotGuiReflectionHelper.invokeBoolean(tab, "setVisible", true);
        SnapshotGuiReflectionHelper.invokeNoArg(tab, "setFullyOpen");
        SnapshotGuiReflectionHelper.invokeNoArg(tab, "update");
    ...
    private Rect getTabBounds(Object tab) {
        try {
            Method method = tab.getClass().getMethod("getBounds");
            Object bounds = method.invoke(tab);
            if (bounds == null) {
                return null;
            }
            Object x = SnapshotGuiReflectionHelper.getFieldValue(bounds, "x");
            Object y = SnapshotGuiReflectionHelper.getFieldValue(bounds, "y");
            Object w = SnapshotGuiReflectionHelper.getFieldValue(bounds, "w");
            Object h = SnapshotGuiReflectionHelper.getFieldValue(bounds, "h");
